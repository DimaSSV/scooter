...
Функция DownloadGET(Запрос)
	...
	Попытка
		...
		Справочники.скт_Методы.Вызвать(Ссылка, Ответ.ПолучитьТелоКакПоток(), Параметры, ФорматРезультата, Запрос);
...
Функция DocGET(Запрос)
	Ответ = Новый HTTPСервисОтвет(200);
	
	Попытка
		
		ОписаниеСервиса = скт_Документация.OpenAPI();
		Ответ.УстановитьТелоИзСтроки(ОписаниеСервиса);
		скт_ПроцедурыHTTP.УстановитьТипКонтентаПоРасширениюФайла(Ответ, "json");
