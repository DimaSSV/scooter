...
Функция ПроверитьЗапуститьТолстыйКлиент(ЭтотОбъект = Неопределено) Экспорт 
	...
	#Если Клиент  Тогда 
		ПоказатьОповещениеПользователя(НСтр("ru = 'Запуск толстого клиента';
											|bg = 'Стартиранее на голям клиент';
											|en = 'Start thick client'"),, НСтр("ru = 'Функция доступна только в толстом клиенте. Поэтому запускается толстый клиент.';
