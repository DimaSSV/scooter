...
Асинх Процедура ПроверитьОбновление(Команда)
	...
	текстВопроса = СтрШаблон("Доступна новая версия подсистемы ""Самокат"". Выполнить обновление?");
	Ответ = Ждать ВопросАсинх(текстВопроса, РежимДиалогаВопрос.ДаНет);
	Если Ответ <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	ОбновитьВерсиюПодсистемыАсинх(АктуальныйРелиз);
...
Процедура ОбновитьВерсиюПодсистемыСервер(УРЛ)
	скт_ПоддержкаСервер.ОбновитьВерсию(УРЛ);
