...
//
// Параметры:
//  Метод				 - СправочникСсылка.скт_Методы - Ссылка на метод, который планируется вызвать
...
//
Процедура Вызвать(Метод, ПотокПриемник, Знач ВходныеПараметры = Неопределено, выхРасширениеФайла = Неопределено, ЗапросHTTP = Неопределено) Экспорт 
	...
	Если Логировать Тогда
		ЗалогироватьНачалоВызова(Метод, Аргументы, ЗапросHTTP);
	КонецЕсли;
	...
	Если Аргументы.СпособДоставки = Перечисления.скт_СпособыДоставкиРезультата.Непосредственно Тогда
		ВывестиРезультатВыполненияПоФормату(РезультатКомпоновки, Метод, Аргументы.Формат, ПотокПриемник, Аргументы.Архивировать, РасширениеФайла);
	...
	Иначе
		...
		ВывестиРезультатВыполненияПоФормату(РезультатКомпоновки, Метод, Аргументы.Формат, Поток, Аргументы.Архивировать, РасширениеФайла);
	...
	Если Логировать Тогда
		ЗалогироватьОкончаниеВызова(Метод, РезультатКомпоновки, РасширениеФайла, ЗапросHTTP);
	КонецЕсли;
...
Процедура ВывестиРезультатВыполненияПоФормату(РезультатВыполнения, Метод, Формат, Поток, Архивировать, выхРасширениеФайла)
	
	Если Архивировать Тогда
		ВременныйФайл = СтрШаблон("%1%2.%3", КаталогВременныхФайлов(), Метод.Код, Перечисления.скт_ВариантыФорматовВыгрузки.РасширениеФайлаДляСохранения(Формат));
...
Процедура ВыполнитьПроизвольнуюСериализацию(РезультатВыполнения, Метод, Поток, выхРасширениеФайла)
	...
	Алгоритм = скт_ПроизвольныеАлгоритмыСервер.ОбъектВнешнейОбработки(Метод.Сериализация);
...
Функция ПолучитьРезультатКомпоновки(Метод, ВходныеПараметры = Неопределено)
	...
	Для Каждого текАргумент Из ТаблицаАргументов Цикл
		...
		Если СтрокаПараметра <> Неопределено Тогда
			Если Не ПустаяСтрока(СтрокаПараметра.ИдентификаторПользовательскойНастройки) Тогда
				ПользовательскаяНастройка = Компоновщик.ПользовательскиеНастройки.Элементы.Найти(СтрокаПараметра.ИдентификаторПользовательскойНастройки);
	...
	Если ВходныеПараметры.Формат = Перечисления.скт_ВариантыФорматовВыгрузки.CSV 
		...
		Или ВходныеПараметры.Формат = Перечисления.скт_ВариантыФорматовВыгрузки.XML 
		Или ВходныеПараметры.Формат = Перечисления.скт_ВариантыФорматовВыгрузки.ПроизвольнаяСериализация Тогда
