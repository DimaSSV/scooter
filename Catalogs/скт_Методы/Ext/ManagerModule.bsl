...
Процедура ВывестиРезультатВыполненияПоФормату(РезультатВыполнения, Метод, Формат, 
	...
	Иначе
		...
		Иначе
			скт_ОбщегоНазначенияСервер.ДвоичныеДанныеТабличногоДокумента(РезультатВыполнения, 
...
Процедура ЗалогироватьОкончаниеВызова(Метод, РезультатКомпоновки, РасширениеФайла, ЗапросHTTP = Неопределено)
	...
	скт_ОбщегоНазначенияСервер.ЗаписатьВЖурнал("Окончание вызова метода", СообщениеДляЖурнала, , 
		Метод, Метод.Метаданные());
