...
&НаКлиенте
Процедура АргументыПрименятьКНастройкеОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Идентификатор = СтрокаВИдентификаторНастройки(ВыбранноеЗначение);
	ЗначениеНастройки = Компоновщик.ПользовательскиеНастройки.ПолучитьОбъектПоИдентификатору(Идентификатор);
	
	СтрокаАргумента = Объект.Аргументы.НайтиПоИдентификатору(Элементы.Аргументы.ТекущаяСтрока);
	СтрокаАргумента.ИдентификаторНастройки = ВыбранноеЗначение;
...
Процедура ПриСозданииПриЧтенииНаСервере(ПараметрКопирования = Неопределено)
	...
	АдресПубликации = скт_Настройки.ПрочитатьЗначение(ПланыВидовХарактеристик.скт_ДополнительныеНастройки.скт_АдресПубликацииБазы, "");
	АдресМетода = АдресМетода(АдресПубликации, Объект.Код);
	Для Каждого текАргумент Из Объект.Аргументы Цикл
		ОбработатьЗаполнениеСтрокиАргумента(текАргумент, Компоновщик);
	КонецЦикла;
...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.АргументыПрименятьКНастройке.СписокВыбора.Очистить();
	Для Каждого ТекНастройка Из Компоновщик.ПользовательскиеНастройки.Элементы Цикл
		
		ТипНастройки = ТипЗнч(ТекНастройка);
		Если ТипНастройки = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
			
			Идентификатор = Компоновщик.ПользовательскиеНастройки.ПолучитьИдентификаторПоОбъекту(ТекНастройка);
			Значение = ИдентификаторНастройкиВСтроку(Идентификатор);
			СтрокаКомпоновки = Компоновщик.ПользовательскиеНастройки.ПолучитьОбъектПоИдентификатору(Идентификатор);
			
			Элементы.АргументыПрименятьКНастройке.СписокВыбора.Добавить(Значение, СтрокаКомпоновки.Параметр,
				, БиблиотекаКартинок.ПараметрыДанныхКомпоновкиДанных);
			
		ИначеЕсли 	ТипНастройки = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			
			Идентификатор = Компоновщик.ПользовательскиеНастройки.ПолучитьИдентификаторПоОбъекту(ТекНастройка);
			Значение = ИдентификаторНастройкиВСтроку(Идентификатор);
			СтрокиКомпоновки = Компоновщик.ПользовательскиеНастройки.ПолучитьОсновныеНастройкиПоИдентификаторуПользовательскойНастройки(Идентификатор);
			Если СтрокиКомпоновки.Количество() Тогда
				СтрокаКомпоновки = СтрокиКомпоновки.Получить(0);
				Элементы.АргументыПрименятьКНастройке.СписокВыбора.Добавить(Значение, СтрокаКомпоновки.ЛевоеЗначение,
...
Процедура СохранитьАдресПубликации(Адрес)
	...
	скт_Настройки.УстановитьЗначение(ПланыВидовХарактеристик.скт_ДополнительныеНастройки.скт_АдресПубликацииБазы, Адрес);
