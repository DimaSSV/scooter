...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Типзнч(ВыбранноеЗначение) = Тип("СхемаКомпоновкиДанных") Тогда
		скт_КомпоновкаДанныхКлиентСервер.УстановитьСхемуКомпоновкиВФорме(ЭтотОбъект, 
			ВыбранноеЗначение, АдресСхемы, Компоновщик);
...
Процедура ОшибкаЗапросаНажатие(Элемент, СтандартнаяОбработка)
	...
	Элементы.ТекстЗапроса.УстановитьГраницыВыделения(Число(МассивКоординат[0]), Число(МассивКоординат[1]), 
		Число(МассивКоординат[0]), Число(МассивКоординат[1]) + ДлинаСтрокиОшибки);
...
Процедура РедактироватьСоставПользовательскихНастроек(Команда)
	...
	ОткрытьФорму("Справочник.скт_Источники.Форма.СоставПользовательскихНастроек", 
		ПараметрыФормы, ЭтаФорма, , , , Обработчик);
...
Процедура ПослеРедактированияСоставаПользовательскихНастроек(Результат, ДопПараметры) Экспорт
	...
	Если Результат.ВыбранныеПоля = ПустаяСтрока(Компоновщик.Настройки.Выбор.ИдентификаторПользовательскойНастройки) Тогда
		Компоновщик.Настройки.Выбор.ИдентификаторПользовательскойНастройки 
			= ?(Результат.ВыбранныеПоля, Новый УникальныйИдентификатор, "");		
		Модифицированность = Истина;
	КонецЕсли;
	
	Если Результат.Отбор = ПустаяСтрока(Компоновщик.Настройки.Отбор.ИдентификаторПользовательскойНастройки) Тогда
		Компоновщик.Настройки.Отбор.ИдентификаторПользовательскойНастройки 
			= ?(Результат.Отбор, Новый УникальныйИдентификатор, "");		
		Модифицированность = Истина;
	КонецЕсли;
	
	Если Результат.Порядок = ПустаяСтрока(Компоновщик.Настройки.Порядок.ИдентификаторПользовательскойНастройки) Тогда
		Компоновщик.Настройки.Порядок.ИдентификаторПользовательскойНастройки 
			= ?(Результат.Порядок, Новый УникальныйИдентификатор, "");		
		Модифицированность = Истина;
	КонецЕсли;
	
	Если Результат.УсловноеОформление 
		= ПустаяСтрока(Компоновщик.Настройки.УсловноеОформление.ИдентификаторПользовательскойНастройки) Тогда
		Компоновщик.Настройки.УсловноеОформление.ИдентификаторПользовательскойНастройки 
			= ?(Результат.УсловноеОформление, Новый УникальныйИдентификатор, "");		
		Модифицированность = Истина;
	КонецЕсли;
	
	Если Результат.Структура = ПустаяСтрока(Компоновщик.Настройки.Структура.ИдентификаторПользовательскойНастройки) Тогда
		Компоновщик.Настройки.Структура.ИдентификаторПользовательскойНастройки 
			= ?(Результат.Структура, Новый УникальныйИдентификатор, "");		
		Модифицированность = Истина;
	КонецЕсли;
...
Процедура ЗаменитьСКДПоТекстуЗапроса()
	...
	Если ЭтоАдресВременногоХранилища(АдресСхемы) Тогда
		Схема = ПолучитьИзВременногоХранилища(АдресСхемы);
		Наборы = ПолучитьИзВременногоХранилища(АдресСхемы).НаборыДанных;
		Если Не Схема.ИсточникиДанных.Количество() Тогда 
			ИсточникДанных = Схема.ИсточникиДанных.Добавить();
			ИсточникДанных.Имя = "ИсточникДанных1";
			ИсточникДанных.ТипИсточникаДанных = "Local";
		КонецЕсли;
		Если Не Наборы.Количество() Тогда
			Набор = Наборы.Добавить(Тип("НаборДанныхЗапросСхемыКомпоновкиДанных"));
			Набор.Имя = "НаборДанных1";
			Набор.ИсточникДанных = Схема.ИсточникиДанных.Получить(0).Имя;
			Набор.АвтоЗаполнениеДоступныхПолей = Истина;
		Иначе
			Набор = Наборы.Получить(0);
		КонецЕсли;
